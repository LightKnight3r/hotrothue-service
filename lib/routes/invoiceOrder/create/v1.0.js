...
module.exports = (req, res) => {
  ...
  const pendingPaymentMethods = ['bank']
  const allowMethods = ['inapp', ...pendingPaymentMethods]
  ...
  const checkParams = (next) => {
    ...
    if(!paymentMethod || !allowMethods.includes(paymentMethod)) {
      return next({
        code: CONSTANTS.CODE.WRONG_PARAMS,
        message: {
          head: 'Thông báo',
          body: 'Bạn vui lòng chọn phương thức thanh toán'
        }
      });
    }
    let resultSummary = {};
    ...
    if(pendingPaymentMethods.includes(req.body.paymentMethod)) {
      req.body.status = CONSTANTS.ORDER_STATUS.WAITING_PAYMENT
    } else if(req.body.paymentMethod === 'inapp') {
      req.body.status = CONSTANTS.ORDER_STATUS.PENDING_RECEIVE
    }
  ...
  const handleCheckData = (next) => {
    ...
    if (duplicateTINs.length > 0) {
      return next({
        ...
        message: {
          head: 'Thông báo',
          body: `Danh sách Mã số thuế không được trùng nhau. Vui lòng kiểm tra lại Mã số thuế: ${duplicateTINs.join(', ')}`
        },
  ...
  const checkAndHandlePaymentViaApp = (next) => {
    ...
    const totalFeeCash = req.body.pricing.totalPrice;
        ...
        .exec((err, result) => {
          ...
          if(!result) {
            return done({
              ...
              message: {
                'head': 'Thông báo',
                'body': `Số dư Tài khoản của bạn hiện không còn đủ ${totalFeeCash.toLocaleString().replace(/,/g, ".")}₫ để thanh toán cho đơn hàng. Vui lòng chọn phương thức thanh toán khác. Xin cảm ơn.`
              }
            })
          }
    ...
    async.waterfall([
      ...
      req.body.paymentStatus = CONSTANTS.PAYMENT_STATUS.PAID;
      next();
    })
  }
