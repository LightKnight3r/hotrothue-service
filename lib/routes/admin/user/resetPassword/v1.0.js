...
module.exports = (req, res) => {
  const { _id, confirmPassword, confirmLevel2Password } = req.body || '';
  ...
  const checkConfirmPassword = (next) => {
    User.findById(userId)
      ...
      .exec((err, result) => {
        ...
        async.parallel(
          [
            (callback) => {
              bcrypt.compare(confirmPassword, result.password, (err, isMatch) => {
                if (err) {
                  return callback(err);
                }
                if (!isMatch) {
                  return callback({
                    ...
                    message: {
                      head: 'Thông báo',
                      body: 'Mật khẩu xác nhận không đúng',
                    },
                  });
                }
                callback();
            ...
            (callback) => {
              bcrypt.compare(confirmLevel2Password, result.passwordLevel2, (err, isMatch) => {
                if (err) {
                  return callback(err);
                }
                if (!isMatch) {
                  return callback({
                    ...
                    message: {
                      head: 'Thông báo',
                      body: 'Mật khẩu cấp 2 xác nhận không đúng',
                    },
                  });
                }
                callback();
    ...
    redisConnection('master')
      ...
      .get(`user:${_id}`, (err, token) => {
        if (token) {
          redisConnection('master')
            .getConnection()
            .del([`user:${token}`, `user:${_id}`], (err, result) => {
              // Session đã được xóa
            });
        }
        next(null);
