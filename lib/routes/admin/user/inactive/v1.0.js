...
module.exports = (req, res) => {
  ...
  const checkParams = (next) => {
    ...
    if (id === userId) {
      return next({
        ...
        message: {
          head: 'Thông báo',
          body: 'Bạn không thể vô hiệu hóa chính mình',
        },
      });
    }
    next(null);
  ...
  const updateRedisData = (next) => {
    ...
    redisConnection('master')
      .getConnection()
      .get(`user:${id}`, (err, token) => {
        if (token) {
          redisConnection('master')
            .getConnection()
            .del([`user:${token}`, `user:${id}`], (err, result) => {});
        }
        next(null);
      });
  };
  ...
  const writeLog = (next) => {
    const logData = {
      user: userId,
      action: active ? 'ACTIVATE_USER' : 'INACTIVE_USER',
      description: active ? 'Kích hoạt người dùng' : 'Vô hiệu hóa người dùng',
      data: {
        userId: id,
        userInfo: {
  ...
  async.waterfall([checkParams, checkUserExists, modifyUser, updateRedisData, writeLog], (err, data) => {
    if (_.isError(err)) {
