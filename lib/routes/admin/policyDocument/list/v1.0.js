...
module.exports = (req, res) => {
  ...
  const listPolicyDocument = (next) => {
    ...
    PolicyDocumentModel.find(query)
      .limit(limit)
      .skip(page * limit)
      ...
      .exec((err, result) => {
        ...
        PolicyDocumentModel.countDocuments(query, (countErr, total) => {
          if (countErr) {
  ...
  async.waterfall([checkParams, listPolicyDocument], (err, data) => {
    err &&
      _.isError(err) &&
      (data = {
