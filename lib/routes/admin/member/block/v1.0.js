...
module.exports = (req, res) => {
  const { id, blockUtil, reason } = req.body || '';
  ...
  const writeLog = (next) => {
    const logData = {
      user: userId,
      action: blockUtil > 0 ? 'BLOCK_MEMBER' : 'UNBLOCK_MEMBER',
      description: reason || (blockUtil > 0 ? 'Khóa tài khoản thành viên' : 'Mở khóa tài khoản thành viên'),
      data: {
        memberId: id,
