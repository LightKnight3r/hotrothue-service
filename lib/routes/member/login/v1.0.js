...
module.exports = (req, res) => {
  ...
  const getFeaturePolicies = (next) => {
    memberInf.tier = memberInf.expiredTime >= Date.now() ? 'MEMBER' : "FREE";
    FeaturePolicyModel.findOne({ scopeValue: memberInf.tier })
      .lean()
      .exec((err, policy) => {
        if (err) {
          return next(err);
        ...
        memberInf.features = policy.features || [];
        next();
      });
  };
  ...
  const createNewToken = (next) => {
    ...
    const objSign = {
      id: userId,
      expiredTime: memberInf.expiredTime
    };
  ...
  async.waterfall([checkParams, findUser, deleteOldToken, getFeaturePolicies, createNewToken], (err, data) => {
    if (_.isError(err)) {
